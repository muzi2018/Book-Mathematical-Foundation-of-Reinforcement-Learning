...
if __name__ == "__main__":      
    ...
    # Initialize the Q-values dictionary with zeros
    q_values = {state: {action: 0.0 for action in actions} for state in states}
# Function to calculate Q-values based on the Bellman optimality equation
    def calculate_q_values(env, states, actions, gamma):
        # Loop over all states to calculate Q-values for each action
        for state in states:
            state_coordinates = states[state]
            
            # For each action, compute the expected Q-value
            for action, action_idx in actions.items():
                # Simulate taking the action and receiving a reward (This part should be customized based on your environment)
                next_state, reward, done, _ = env.step(action_idx)
                next_state_coords = (next_state[0], next_state[1])  # Assuming `next_state` provides coordinates
                
                # Calculate the Q-value based on the Bellman optimality equation
                # Assuming `env.get_transition_prob` and `env.get_reward` are defined appropriately
                max_q_next = max(q_values[next_state][a] for a in actions)  # max_a' Q(s', a')
                q_values[state][action] = reward + gamma * max_q_next
                
        return q_values
    # Function to find the optimal policy based on Q-values
    def find_optimal_policy(q_values, states, actions):
        optimal_policy = {}
        for state in states:
            # Find the action that maximizes Q(s, a)
            best_action = max(actions, key=lambda a: q_values[state][a])
            optimal_policy[state] = best_action
        return optimal_policy

    # Example: Update Q-values and find the optimal policy
    q_values = calculate_q_values(env, states, actions, gamma_)
    optimal_policy = find_optimal_policy(q_values, states, actions)
